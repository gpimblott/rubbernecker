...
Handlebars.registerHelper("story_summary", function (stories) {
    ...
    var percentageFinished = (stories.length == 0 || finished == 0 ? 0 : (100 / (stories.length / finished)));
    var percentageStarted = (stories.length == 0 || started == 0 ? 0 : (100 / (stories.length / started)));
    var percentageNotStarted = (stories.length == 0 || notStarted == 0 ? 0 : (100 / (stories.length / notStarted)));

    var text = '<small>' + ' Not Started:' + (unstarted + unscheduled)
        + ' Started:' + started
        + ' Finished:' + finished
        + '</small>';
    ...
    text += '<div class="progress">'
        ...
        + Math.floor(percentageNotStarted) + '% not started'
        + '</div>'
...
Handlebars.registerHelper("round_down", function (num) {
